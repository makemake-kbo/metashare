...
void usage(const char* argv0) {
    std::fprintf(
        ...
        "Usage: %s [options]\n"
        "  --mode <name>        portal-virtual | portal-monitor | "
        "mutter-direct\n"
        "                       (default: portal-virtual)\n"
        "  --output <path>      output PPM path (default: metashare-dump.ppm)\n"
        "  --fps <n>            capture framerate hint (default 60)\n"
        "  --width <px>         mutter-direct virtual monitor width (default "
        "1280)\n"
        "  --height <px>        mutter-direct virtual monitor height (default "
        "720)\n"
        "  -h, --help           this help\n\n"
        "Requires GNOME 44+. portal-* modes show a one-time confirmation "
        "dialog.\n",
        argv0);
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-h" || a == "--help") {
            ...
            if (auto* v = next("--width"))
                mon_w = std::atoi(v);
            else
                return 2;
        } else if (a == "--height") {
            if (auto* v = next("--height"))
                mon_h = std::atoi(v);
            else
                return 2;
        } else {
    ...
    std::unique_ptr<MutterScreenCastSession>
        mutter_session;  // keeps session alive
    ...
    for (int tries = 0; tries < 20 && !frame; ++tries) {
        ...
        if (r < 0) {
            std::fprintf(stderr,
                         "[dump] source ended before a frame arrived\n");
            source->stop();
            return 1;
        }
    }
