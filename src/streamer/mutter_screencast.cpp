...
int MutterScreenCastSession::add_virtual_monitor(int width, int height,
                                                 std::string& err) {
    if (!open_) {
        err = "session not open";
        return -1;
    }
    if (width <= 0 || height <= 0) {
        err = "invalid monitor size";
        return -1;
    }
    ...
    try {
        ...
        std::fprintf(stderr, "[mutter] virtual monitor %d: %dx%d -> %s\n", idx,
                     width, height, monitors_.back().stream_path.c_str());
        return idx;
    } catch (const sdbus::Error& e) {
...
void MutterScreenCastSession::close() {
    ...
    if (!session_path_.empty()) {
        ...
        } catch (...) {  // NOLINT(bugprone-empty-catch) — best-effort teardown
        }
    }
    session_path_.clear();
    monitors_.clear();
