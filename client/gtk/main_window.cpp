...
MainWindow::~MainWindow() {
  // Make sure we don't leave a streamer process orphaned if the user closes the
  // window mid-stream.
...
Gtk::ListBoxRow* MainWindow::make_row(const Glib::ustring& title,
  ...
  auto* title_box = Gtk::make_managed<Gtk::Box>(Gtk::Orientation::HORIZONTAL, 12);
...
void MainWindow::build_ui() {
  ...
  btn_start_.signal_clicked().connect(sigc::mem_fun(*this, &MainWindow::on_start_clicked));
  ...
  // --- Source ---
  {
    drop_source_.set_model(Gtk::StringList::create({"Test pattern", "Wayland portal"}));
    drop_source_.set_tooltip_text("Test pattern draws a synthetic moving image — use it when developing without a Wayland session. Portal captures a real Wayland monitor via xdg-desktop-portal.");
    drop_source_.property_selected().signal_changed().connect(
        sigc::mem_fun(*this, &MainWindow::on_source_changed));
    list->append(*make_row("Capture source",
  ...
  // --- Aspect ratio preset ---
  {
    ...
    list->append(*make_row("Aspect ratio",
  ...
  // --- Width / Height ---
  {
    ...
    auto* dims = Gtk::make_managed<Gtk::Box>(Gtk::Orientation::HORIZONTAL, 6);
    ...
    list->append(*make_row("Resolution",
  ...
  // --- Lock aspect switch ---
  {
    ...
    list->append(*make_row("Lock aspect ratio",
                           "When on, editing one dimension updates the other.",
                           sw_lock_));
  }
  ...
  // --- FPS ---
  {
    ...
    list->append(*make_row("Frame rate",
  ...
  // --- Codec ---
  {
    drop_codec_.set_model(Gtk::StringList::create({"HEVC (H.265)", "H.264"}));
    list->append(*make_row("Codec",
                           "HEVC falls back to software H.264 if no HW HEVC encoder is available.",
                           drop_codec_));
  }
  ...
  // --- Hardware accel ---
  {
    ...
    list->append(*make_row("Hardware encoder",
                           "Use VAAPI/NVENC when available. Disable to force software encoding.",
                           sw_hw_));
  }
  ...
  // --- Discovery ---
  {
    ...
    list->append(*make_row("UDP discovery",
  ...
  {
    entry_streamer_path_.set_placeholder_text("auto (" + Glib::ustring(kStreamerBinary) + ")");
    entry_streamer_path_.set_width_chars(20);
    list->append(*make_row("Streamer binary",
...
Glib::ustring MainWindow::resolve_streamer_path() const {
  const Glib::ustring ov = entry_streamer_path_.get_text();
  if (!ov.empty()) {
...
std::vector<std::string> MainWindow::build_argv() const {
  ...
  if (!portal) {
    ...
    argv.push_back(std::to_string(round_even(spin_width_.get_value_as_int())));
    argv.push_back("--height");
    argv.push_back(std::to_string(round_even(spin_height_.get_value_as_int())));
  }
...
void MainWindow::start_streamer() {
  ...
  if (argv.empty() || argv.front().empty()) {
    append_log("Could not find the streamer binary.\n"
               "Install metashare-streamer or set a path under 'Streamer binary'.\n");
    return;
  }
  ...
  try {
    Glib::spawn_async_with_pipes(
        ...
        /* child_setup */ sigc::slot<void()>(),
        &pid, /* stdin */ nullptr, &out_fd, &err_fd);
  ...
  if (stdout_fd_ >= 0) {
    io_stdout_conn_ = Glib::signal_io().connect(
        sigc::bind(sigc::mem_fun(*this, &MainWindow::on_pipe_io), stdout_fd_),
        stdout_fd_,
  ...
  if (stderr_fd_ >= 0) {
    io_stderr_conn_ = Glib::signal_io().connect(
        sigc::bind(sigc::mem_fun(*this, &MainWindow::on_pipe_io), stderr_fd_),
        stderr_fd_,
        Glib::IOCondition::IO_IN | Glib::IOCondition::IO_HUP |
            Glib::IOCondition::IO_ERR);
  }
...
void MainWindow::stop_streamer() {
  ...
  // The actual cleanup happens in on_child_exited(). If the child doesn't exit
  // within a couple of seconds, escalate to SIGKILL.
...
bool MainWindow::on_pipe_io(Glib::IOCondition cond, int fd) {
  ...
  do {
    ...
    if (n > 0) {
      append_log(Glib::ustring(buf, buf + n));
    }
  } while (n == static_cast<ssize_t>(sizeof(buf)));  // drain fully

  if (n <= 0) {
    // EOF or error: drop this source. The fd itself is closed in on_child_exited.
...
void MainWindow::apply_running_state(bool running) {
  ...
  // Lock all settings while the streamer is running — changing them would need
  // a restart anyway.
